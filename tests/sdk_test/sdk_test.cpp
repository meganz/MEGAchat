...
TEST_F(MegaChatApiTest, WaitingRooms)
{
    ...
    *previewsUpdated = false;
    ...
    LOG_debug << "A kicks (completely disconnect) B from call (again) and then ends call for all participants";
    kickAndEndCall(auxCall->getCallId());

    // [Test5]: A starts call relying on waiting room flag from chatroom.
    ...
    // --------------------------------------------------------------------------------------------------------------
    LOG_debug << "Test5: A starts call with waiting room, B is redirected to waiting room";
