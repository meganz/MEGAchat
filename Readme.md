...
MEGAchat also needs the MEGA SDK library. There are instructions later in this doument on how to get it to be used with MEGAchat. The MEGA SDK project is automatically loaded by the MEGAchat CMake, so you only have to simply clone it in the expect path.
