...
void process_line(const char* l)
{
    ...
    case PIN:
    {
        std::string pin = l;
        g_chatApi->init(NULL);
        g_megaApi->multiFactorAuthLogin(g_login.c_str(), g_password.c_str(), !pin.empty() ? pin.c_str() : NULL);
        {
            clc_console::conlock(std::cout) << "\nLogging in with 2FA..." << std::endl << std::flush;
        }
        setprompt(NOPROMPT);
        return;
    }
    ...
    case COMMAND:
        try
        {
            ...
            ac::autoExec(std::string(l), std::string::npos, g_autocompleteTemplate, false, consoleOutput, true); // todo: pass correct unixCompletions flag
