...
char** my_rl_completion(const char*, int, int end)
{
    ...
    ac::CompletionState acs =
        ac::autoComplete(line, line.size(), clc_global::g_autocompleteTemplate, true);
...
ac::ACN autocompleteSyntax()
{
    ...
    p->Add(exec_login,
           sequence(text("login"),
                    either(sequence(param("email"), opt(param("password"))),
                           param("session"),
    ...
    p->Add(exec_setonlinestatus,
           sequence(text("setonlinestatus"),
                    either(text("offline"), text("away"), text("online"), text("busy"))));
    p->Add(exec_setpresenceautoaway,
           sequence(text("setpresenceautoaway"), either(text("on"), text("off")), wholenumber(30)));
    p->Add(exec_setpresencepersist,
           sequence(text("setpresencepersist"), either(text("on"), text("off"))));
    ...
    p->Add(exec_getinactivechatlistitems,
           sequence(text("getinactivechatlistitems"), param("roomid")));
    ...
    p->Add(exec_createchat,
           sequence(text("createchat"),
                    opt(flag("-group")),
                    opt(flag("-public")),
                    opt(flag("-meeting")),
                    repeat(param("userid"))));
    ...
    p->Add(exec_updatechatpermissions,
           sequence(text("updatechatpermissions"), param("roomid"), param("userid")));
    ...
    p->Add(exec_setRetentionTime,
           sequence(text("setretentiontime"), param("roomid"), param("period")));
    ...
    p->Add(exec_loadmessages,
           sequence(text("loadmessages"),
                    param("roomid"),
                    wholenumber(10),
    ...
    p->Add(exec_getmanualsendingmessage,
           sequence(text("getmanualsendingmessage"), param("roomid"), param("tempmsgid")));
    p->Add(exec_sendmessage, sequence(text("sendmessage"), param("roomid"), param("text")));
    p->Add(exec_attachcontacts,
           sequence(text("attachcontacts"), param("roomid"), repeat(param("userid"))));
    p->Add(exec_attachnode, sequence(text("attachnode"), param("roomid"), param("nodeid")));
    p->Add(exec_revokeattachmentmessage,
    ...
    p->Add(exec_removeunsentmessage,
           sequence(text("removeunsentmessage"), param("roomid"), param("tempid")));
    p->Add(exec_sendtypingnotification, sequence(text("sendtypingnotification"), param("roomid")));
    p->Add(exec_ismessagereceptionconfirmationactive,
           sequence(text("ismessagereceptionconfirmationactive")));
    ...
    p->Add(exec_startchatcall,
           sequence(text("startchatcall"),
                    opt(flag("-novideo")),
                    opt(flag("-noaudio")),
                    param("roomid")));
    p->Add(exec_answerchatcall,
           sequence(text("answerchatcall"),
                    opt(flag("-novideo")),
                    opt(flag("-noaudio")),
                    param("roomid")));
    ...
    p->Add(exec_smsverify,
           sequence(text("smsverify"),
                    either(sequence(text("send"), param("phoneNumber"), opt(text("to"))),
                           sequence(text("code"), param("code")),
                           text("allowed"),
                           text("phone"))));
    ...
    p->Add(exec_recentactions,
           sequence(text("recentactions"), opt(sequence(param("days"), param("nodecount")))));
    p->Add(exec_getspecificaccountdetails,
           sequence(text("getspecificaccountdetails"),
                    repeat(either(flag("-storage"), flag("-transfer"), flag("-pro")))));

    p->Add(exec_backgroundupload,
           sequence(text("backgroundupload"),
                    either(sequence(text("new"), param("name")),
                           sequence(text("resume"), param("name"), param("serializeddata")),
                           sequence(text("analyse"), param("name"), localFSFile()),
                           sequence(text("encrypt"),
                                    param("name"),
                                    localFSFile(),
                                    localFSFile(),
                                    param("startPos"),
                                    param("length"),
                                    either(text("false"), text("true"))),
                           ...
                           sequence(text("setcoordinates"),
                                    param("name"),
                                    opt(flag("-shareable")),
                                    param("latitude"),
                                    param("longitude")),
                           sequence(text("complete"),
                                    param("name"),
                                    param("nodename"),
    ...
    p->Add(exec_getfingerprint,
           sequence(text("getfingerprint"),
                    either(sequence(text("local"), localFSFile()),
                           sequence(text("remote"), param("remotefile")),
                           sequence(text("original"), param("remotefile")))));

    p->Add(exec_setthumbnailbyhandle,
           sequence(text("setthumbnailbyhandle"), param("remotepath"), param("attributehandle")));
    p->Add(exec_setpreviewbyhandle,
           sequence(text("setpreviewbyhandle"), param("remotepath"), param("attributehandle")));
    p->Add(exec_setnodecoordinates,
           sequence(text("setnodecoordinates"),
                    param("remotepath"),
                    param("latitude"),
                    param("longitude")));
    p->Add(exec_setunshareablenodecoordinates,
           sequence(text("setunshareablenodecoordinates"),
                    param("remotepath"),
                    param("latitude"),
                    param("longitude")));
    p->Add(exec_createthumbnail,
           sequence(text("createthumbnail"),
                    opt(flag("-tempmegaapi")),
                    opt(sequence(flag("-parallel"), param("count"))),
                    localFSFile(),
                    localFSFile()));
    ...
    p->Add(exec_ls,
           sequence(text("ls"),
                    repeat(either(flag("-recursive"),
                                  flag("-handles"),
                                  flag("-ctime"),
                                  flag("-mtime"),
                                  flag("-size"),
                                  flag("-versions"),
                                  sequence(flag("-order"), param("order")),
                                  sequence(flag("-refilter"), param("regex")))),
                    param("path")));
    ...
    p->Add(exec_startupload,
           sequence(text("startupload"),
                    localFSPath(),
                    param("remotepath"),
                    opt(flag("-withcanceltoken")),
                    opt(flag("-logstage")),
                    opt(sequence(flag("-filename"), param("newname")))));
    p->Add(exec_startdownload,
           sequence(text("startdownload"),
                    param("remotepath"),
                    localFSPath(),
                    opt(flag("-withcanceltoken")),
                    (flag("-logstage"))));
    p->Add(exec_pausetransfers,
           sequence(text("pausetransfers"), param("pause"), opt(param("direction"))));
    p->Add(exec_pausetransferbytag,
           sequence(text("exec_pausetransferbytag"), param("tag"), param("pause")));
    ...
    p->Add(exec_exportNode,
           sequence(text("exportnode"),
                    opt(sequence(flag("-writable"), either(text("true"), text("false")))),
                    opt(sequence(flag("-expiry"), param("time_t"))),
                    param("remotepath")));

    p->Add(exec_pushreceived,
           sequence(text("pushreceived"), opt(flag("-beep")), opt(param("chatid"))));
    ...
    p->Add(exec_mv,
           sequence(text("mv"),
                    param("remotesrc"),
                    param("remotedst"),
                    opt(sequence(flag("-rename"), param("newname")))));

    p->Add(exec_setCameraUploadsFolder,
           sequence(text("setcamerauploadsfolder"), param("remotedst")));
    p->Add(exec_getCameraUploadsFolder, sequence(text("getcamerauploadsfolder")));
    p->Add(exec_setCameraUploadsFolderSecondary,
           sequence(text("setcamerauploadsfoldersecondary"), param("remotedst")));
    ...
    // Helpers
    p->Add(exec_treecompare,
           sequence(text("treecompare"),
                    localFSPath(),
                    param("remotepath") /*remoteFSPath(client, &cwd)*/));
    p->Add(exec_generatetestfilesfolders,
           sequence(text("generatetestfilesfolders"),
                    repeat(either(sequence(flag("-folderdepth"), param("depth")),
                                  ...
                                  sequence(flag("-nameprefix"), param("prefix")))),
                    localFSFolder("parent")));
    ...
    p->Add(exec_syncclosedrive, sequence(text("sync"), text("closedrive"), localFSFolder("drive")));

    p->Add(exec_syncexport, sequence(text("sync"), text("export"), opt(localFSFile("outputFile"))));

    p->Add(exec_syncimport, sequence(text("sync"), text("import"), localFSFile("inputFile")));

    p->Add(exec_syncopendrive, sequence(text("sync"), text("opendrive"), localFSFolder("drive")));

    p->Add(exec_synclist, sequence(text("sync"), text("list")));

    p->Add(exec_syncremove,
           sequence(text("sync"),
                    text("remove"),
                    either(sequence(flag("-id"), param("backupId")),
                           sequence(flag("-path"), param("targetpath")))));

    p->Add(exec_syncxable,
           sequence(text("sync"), either(text("disable"), text("enable")), param("id")));
