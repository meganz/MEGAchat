...
void OneShotRequestListener::onRequestFinish(m::MegaApi* api, m::MegaRequest *request, m::MegaError* e)
{
    if (onRequestFinishFunc) onRequestFinishFunc(api, request, e);
    delete this;  // one-shot is done so auto-delete
}

void OneShotTransferListener::onTransferFinish(m::MegaApi* api, m::MegaTransfer *request, m::MegaError* e)
{
...
void OneShotTransferListener::onTransferStart(m::MegaApi*, m::MegaTransfer *request)
{
    if (!loggedStart)
    {
        ...
        clc_console::conlock(std::cout) << "transfer starts, tag: " << request->getTag() << ": " << path << std::endl;
    }
}
...
void OneShotTransferListener::onTransferUpdate(m::MegaApi *, m::MegaTransfer *t)
{
    if (mLogStage && lastKnownStage != t->getStage())
    {
        ...
        {
            clc_console::conlock(std::cout) << "Transfer stage: " << t->stageToString(t->getStage()) << std::endl;
        }
    }
}
...
void OneShotChatRequestListener::onRequestFinish(c::MegaChatApi* api, c::MegaChatRequest *request, c::MegaChatError* e)
{
    if (onRequestFinishFunc) onRequestFinishFunc(api, request, e);
    delete this;  // one-shot is done so auto-delete
}
...
void OneShotChatRequestListener::onRequestTemporaryError(c::MegaChatApi *api, c::MegaChatRequest *request, c::MegaChatError* error)
{
    if (onRequestTemporaryErrorFunc) onRequestTemporaryErrorFunc(api, request, error);
}

CLCRoomListenerRecord::CLCRoomListenerRecord() : listener(new CLCRoomListener) {}
...
void CLCListener::onChatConnectionStateUpdate(c::MegaChatApi* api, c::MegaChatHandle chatid, int newState)
{
...
void CLCCallListener::onChatCallUpdate(megachat::MegaChatApi*, megachat::MegaChatCall *call)
{
    ...
    if (call->hasChanged(megachat::MegaChatCall::CHANGE_TYPE_STATUS))
    {
        ...
        if (status == megachat::MegaChatCall::CALL_STATUS_INITIAL)
        {
            ...
            {
                g_callStateMap.emplace(std::make_pair(chatid, CLCStateChange{megachat::MegaChatCall::CALL_STATUS_INITIAL, true}));
            }
        }
...
void CLCCallListener::onChatSessionUpdate(megachat::MegaChatApi *, megachat::MegaChatHandle chatid, megachat::MegaChatHandle callid, megachat::MegaChatSession *session)
{
    if (!session)
    {
...
void CLCChatListener::onRequestFinish(c::MegaChatApi* api, c::MegaChatRequest *request, c::MegaChatError* e)
{
    assert(request && e);
    ...
    else
    {
        switch (request->getType())
        {
            ...
            case c::MegaChatRequest::TYPE_SET_PRESENCE_AUTOAWAY:
                ...
                {
                    clc_console::conlock(std::cout) << " autoaway: " << request->getFlag() << " timeout: " << request->getNumber() << std::endl;
                }
                break;
            ...
            case c::MegaChatRequest::TYPE_SET_PRESENCE_PERSIST:
                ...
                {
                    clc_console::conlock(std::cout) << " persist: " << request->getFlag() << std::endl;
                }
                break;
            ...
            case c::MegaChatRequest::TYPE_SET_BACKGROUND_STATUS:
                ...
                {
                    clc_console::conlock(std::cout) << " background: " << request->getFlag() << std::endl;
                }
                break;
            ...
            case c::MegaChatRequest::TYPE_LOAD_PREVIEW:
                ...
                {
                    clc_console::conlock(std::cout) << "openchatpreview: chatlink loaded. Chatid: " << karere::Id(request->getChatHandle()).toString() << std::endl;
                }
                break;
        }
    }
...
{
    clc_console::conlock(std::cout) << "User list updated:  " << (users ? users->size() : -1) << std::endl;
    if (users)
    {
        for (int i = 0; i < users->size(); ++i)
        {
            if (m::MegaUser* m = users->get(i))
            {
                ...
                if (changebits)
                {
                    ...
                    if (changebits & m::MegaUser::CHANGE_TYPE_AUTHRING) std::cout << " AUTHRING";
                    if (changebits & m::MegaUser::CHANGE_TYPE_LSTINT) std::cout << " LSTINT";
                    if (changebits & m::MegaUser::CHANGE_TYPE_AVATAR) std::cout << " AVATAR";
                    if (changebits & m::MegaUser::CHANGE_TYPE_FIRSTNAME) std::cout << " FIRSTNAME";
                    if (changebits & m::MegaUser::CHANGE_TYPE_LASTNAME) std::cout << " LASTNAME";
                    if (changebits & m::MegaUser::CHANGE_TYPE_EMAIL) std::cout << " EMAIL";
                    if (changebits & m::MegaUser::CHANGE_TYPE_KEYRING) std::cout << " KEYRING";
                    if (changebits & m::MegaUser::CHANGE_TYPE_COUNTRY) std::cout << " COUNTRY";
                    if (changebits & m::MegaUser::CHANGE_TYPE_BIRTHDAY) std::cout << " BIRTHDAY";
                    if (changebits & m::MegaUser::CHANGE_TYPE_PUBKEY_CU255) std::cout << " PUBKEY_CU255";
                    if (changebits & m::MegaUser::CHANGE_TYPE_PUBKEY_ED255) std::cout << " PUBKEY_ED255";
                    if (changebits & m::MegaUser::CHANGE_TYPE_SIG_PUBKEY_RSA) std::cout << " SIG_PUBKEY_RSA";
                    if (changebits & m::MegaUser::CHANGE_TYPE_SIG_PUBKEY_CU255) std::cout << " SIG_PUBKEY_CU255";
                    if (changebits & m::MegaUser::CHANGE_TYPE_LANGUAGE) std::cout << " LANGUAGE";
                    if (changebits & m::MegaUser::CHANGE_TYPE_PWD_REMINDER) std::cout << " PWD_REMINDER";
                    if (changebits & m::MegaUser::CHANGE_TYPE_DISABLE_VERSIONS) std::cout << " DISABLE_VERSIONS";
                    std::cout << std::endl;
                }
            }
        }
...
{
    clc_console::conlock(std::cout) << "Contact requests list updated:  " << (requests ? requests->size() : -1) << std::endl;
}

void CLCMegaListener::onReloadNeeded(m::MegaApi* api)
{
    {
        clc_console::conlock(std::cout) << "Reload needed!  Submitting fetchNodes request" << std::endl;
    }
    api->fetchNodes();
}
...
{
    clc_console::conlock(std::cout) << "Chats updated:  " << (chats ? chats->size() : -1) << std::endl;
}
...
void CLCMegaListener::onRequestFinish(m::MegaApi* api, m::MegaRequest *request, m::MegaError* e)
{
    ...
    case m::MegaRequest::TYPE_LOGIN:
        ...
        {
            clc_console::conlock(std::cout) << "Loading Account with fetchNodes..." << std::endl;
            guard.unlock();
            api->fetchNodes();
    ...
    case m::MegaRequest::TYPE_LOGOUT:
        ...
        {
            clc_console::conlock(std::cout) << "Error in logout: "<< e->getErrorString() << std::endl;
        }
...
void CLCChatRequestTracker::onRequestFinish(::megachat::MegaChatApi*, ::megachat::MegaChatRequest* req,
