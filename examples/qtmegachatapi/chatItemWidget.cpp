...
void ChatItemWidget::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    auto actAdhocCall = callsMenu->addAction(tr("Add-hoc call"));
    connect(actAdhocCall, SIGNAL(triggered()), mController, SLOT(onAddhocCall()));
    ...
    auto actUpdateMeeting = roomMenu->addAction(tr("Update scheduled meeting"));
    connect(actUpdateMeeting, SIGNAL(triggered()), mController, SLOT(updateScheduledMeeting()));
