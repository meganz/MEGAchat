...
MegaChatApiImpl::~MegaChatApiImpl()
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_delete(request); });
...
void MegaChatApiImpl::sendPendingRequests()
{
    ...
    while (1)
    {
        if (errorCode && request)
        {
            ...
            API_LOG_WARNING("%sError starting request: %s",
                            getLoggingName(),
                            megaChatError->getErrorString());
        ...
        if (!mClient && request->getType() != MegaChatRequest::TYPE_DELETE)
        {
            ...
            API_LOG_WARNING("%sChat engine not initialized yet, cannot process the request",
                            getLoggingName());
            fireOnChatRequestFinish(request, megaChatError);
            continue;
        }
        ...
        {
            MegaChatErrorPrivate *megaChatError = new MegaChatErrorPrivate(MegaChatError::ERROR_ACCESS);
            API_LOG_WARNING("%sChat engine is terminated, cannot process the request",
                            getLoggingName());
            fireOnChatRequestFinish(request, megaChatError);
            continue;
        }

        errorCode = request->hasPerformRequest() ? request->performRequest() : MegaChatError::ERROR_UNKNOWN;
    } // end of while(1)
}

int MegaChatApiImpl::performRequest_retryPendingConnections(MegaChatRequestPrivate *request)
{
...
int MegaChatApiImpl::performRequest_signalActivity(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
...
int MegaChatApiImpl::performRequest_setPresenceAutoaway(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
...
int MegaChatApiImpl::performRequest_setPresencePersist(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            bool enable = request->getFlag();
...
int MegaChatApiImpl::performRequest_setLastGreenVisible(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            bool enable = request->getFlag();
...
int MegaChatApiImpl::performRequest_lastGreen(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
...
int MegaChatApiImpl::performRequest_logout(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            API_LOG_INFO("%sChat engine is logged out!", getLoggingName());
...
int MegaChatApiImpl::performRequest_delete(MegaChatRequestPrivate */*request*/)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            if (mClient && !mTerminating)
            {
                cleanChatHandlers();
                mClient->terminate();
                API_LOG_INFO("%sChat engine closed!", getLoggingName());
...
int MegaChatApiImpl::performRequest_setOnlineStatus(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
...
int MegaChatApiImpl::performRequest_createChatroom(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
    {
        ...
        auto peerList = request->getMegaChatPeerList();
        auto numPeers = peerList ? peerList->size() : 0;
        if (!group && publicChat)
        {
        ...
        if (!group && numPeers > 1)
        {
            ...
            API_LOG_INFO("%sForcing group chat due to more than 2 participants", getLoggingName());
        }
        ...
        if (group)
        {
            ...
            {
                const MegaChatScheduledMeeting* sm =
                    request->getMegaChatScheduledMeetingList()->at(0);
                if (!sm)
                {
                    API_LOG_ERROR("%sError creating a scheduled meeting: invalid scheduled meeting",
                                  getLoggingName());
                    return MegaChatError::ERROR_ARGS;
                }
                ...
                {
                    API_LOG_ERROR("%sError creating a scheduled meeting: invalid timezone, "
                                  "title, description, start date time or end date time",
                                  getLoggingName());
                    return MegaChatError::ERROR_ARGS;
                }
                ...
                {
                    API_LOG_ERROR("%sError creating a scheduled meeting: title or description "
                                  "length exceeded",
                                  getLoggingName());
                    return MegaChatError::ERROR_ARGS;
                }

                std::unique_ptr<::mega::MegaScheduledFlags> megaFlags(
                    !sm->flags() ? nullptr : ::mega::MegaScheduledFlags::createInstance());
                if (megaFlags)
                {
                    assert(sm->flags());
                    megaFlags->importFlagsValue(
                        static_cast<MegaChatScheduledFlagsPrivate*>(sm->flags())
                            ->getNumericValue());
                }

                std::unique_ptr<::mega::MegaScheduledRules> megaRules(
                    !sm->rules() ?
                        nullptr :
                        ::mega::MegaScheduledRules::createInstance(sm->rules()->freq(),
                                                                   sm->rules()->interval(),
                                                                   sm->rules()->until(),
                                                                   sm->rules()->byWeekDay(),
                                                                   sm->rules()->byMonthDay(),
                                                                   sm->rules()->byMonthWeekDay()));

                megaSchedMeeting.reset(MegaScheduledMeeting::createInstance(sm->chatId(),
                                                                            sm->schedId(),
                                                                            sm->parentSchedId(),
                                                                            sm->organizerUserId(),
                                                                            sm->cancelled(),
            ...
            if (!isValidChatOptionsBitMask(
                    chatOptionsBitMask)) // empty bitmask is considered as a valid value
            {
                return MegaChatError::ERROR_ACCESS;
            }
                ...
                .then(
                        ...
                        std::pair<karere::Id, std::shared_ptr<KarereScheduledMeeting>> res)
                    {
                        ...
                        if (creatingSchedMeeting)
                        {
                            if (res.second)
                            {
                                std::unique_ptr<MegaChatScheduledMeetingList> l(
                                    MegaChatScheduledMeetingList::createInstance());
        ...
        {
            if (!numPeers) // chat with self
            {
                auto& chats = *mClient->chats;
                // chat with self - return it if already exists
                if (chats.selfChat())
                {
                    request->setChatHandle(chats.selfChat()->chatid());
                    fireOnChatRequestFinish(request,
                                            new MegaChatErrorPrivate(MegaChatError::ERROR_OK));
                    return MegaChatError::ERROR_OK;
                }
                auto wptr = mClient->weakHandle();
                mClient->createSelfChat()
                    .then(
                        [this, request, wptr, &chats]()
                        {
                            request->setChatHandle(chats.selfChat()->chatid());
                            fireOnChatRequestFinish(
                                request,
            ...
            else // numPeers > 0, peerList is not null
            {
...
int MegaChatApiImpl::performRequest_setChatroomOptions(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            switch (option)
            {
                ...
                case MegaChatApi::CHAT_OPTION_WAITING_ROOM:
                    ...
                    break;
                default:
                    return MegaChatError::ERROR_ARGS; // unknown chat option
            }
            ...
            ((GroupChatRoom*)chatroom)
                ->setChatRoomOption(option, enabled)
                .then(
                    [request, this]()
                    {
                        MegaChatErrorPrivate* megaChatError =
                ...
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sError setting chatroom option : %s",
                                      getLoggingName(),
                                      err.what());
                        MegaChatErrorPrivate* megaChatError =
                            new MegaChatErrorPrivate(err.msg(), err.code(), err.type());
...
int MegaChatApiImpl::performRequest_inviteToChatroom(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            handle chatid = request->getChatHandle();
            ...
            {
                API_LOG_ERROR(
                    "%sRequest (TYPE_INVITE_TO_CHATROOM). Invalid chatid: %s or userid %s",
                    getLoggingName(),
            ...
            if (!chatroom)
            {
                API_LOG_ERROR("%sRequest (TYPE_INVITE_TO_CHATROOM). Chatroom not found. chatid: %s",
                              getLoggingName(),
                              karere::Id(chatid).toString().c_str());
                return MegaChatError::ERROR_NOENT;
            }
            ...
            {
                API_LOG_ERROR(
                    "%sRequest (TYPE_INVITE_TO_CHATROOM). Chatroom is not groupal. chatid: %s",
                    getLoggingName(),
                    karere::Id(chatid).toString().c_str());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            if (chatroom->ownPriv() < static_cast<Priv>(MegaChatPeerList::PRIV_MODERATOR))
            {
                if (chatroom->ownPriv() < static_cast<Priv>(MegaChatPeerList::PRIV_STANDARD) || !chatroom->isOpenInvite())
                {
                    ...
                    API_LOG_ERROR("%sRequest (TYPE_INVITE_TO_CHATROOM). Insufficient permissions "
                                  "to perform this action, for chat: %s",
                                  getLoggingName(),
                                  karere::Id(chatid).toString().c_str());
                    return MegaChatError::ERROR_ACCESS;
                }
            ...
            ((GroupChatRoom*)chatroom)
                ...
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sRequest (TYPE_INVITE_TO_CHATROOM). Error adding user to "
                                      "group chat: %s",
                                      getLoggingName(),
                                      err.what());
...
int MegaChatApiImpl::performRequest_autojoinPublicChat(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            handle chatid = request->getChatHandle();
            ...
            ((GroupChatRoom*)chatroom)
                ...
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sError joining user to public group chat: %s",
                                      getLoggingName(),
                                      err.what());
                        MegaChatErrorPrivate* megaChatError =
                            new MegaChatErrorPrivate(err.msg(), err.code(), err.type());
...
int MegaChatApiImpl::performRequest_updatePeerPermissions(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            handle chatid = request->getChatHandle();
            ...
            ((GroupChatRoom*)chatroom)
                ...
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sError updating peer privileges: %s",
                                      getLoggingName(),
                                      err.what());
...
int MegaChatApiImpl::performRequest_removeFromChatroom(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            handle chatid = request->getChatHandle();
            ...
            if ( uh == MEGACHAT_INVALID_HANDLE || uh == mClient->myHandle())
            {
                ((GroupChatRoom*)chatroom)
                    ...
                    .fail(
                        [request, this](const ::promise::Error& err)
                        {
                            API_LOG_ERROR("%sError leaving chat: %s", getLoggingName(), err.what());
            ...
            else
            {
                ...
                ((GroupChatRoom*)chatroom)
                    ...
                    .fail(
                        [request, this](const ::promise::Error& err)
                        {
                            API_LOG_ERROR("%sError removing peer from chat: %s",
                                          getLoggingName(),
                                          err.what());
...
int MegaChatApiImpl::performRequest_truncateHistory(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            handle chatid = request->getChatHandle();
            ...
            chatroom->truncateHistory(messageid)
                ...
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sError truncating chat history: %s",
                                      getLoggingName(),
                                      err.what());
...
int MegaChatApiImpl::performRequest_editChatroomName(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            handle chatid = request->getChatHandle();
            ...
            ((GroupChatRoom*)chatroom)
                ...
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sError editing chat title: %s",
                                      getLoggingName(),
                                      err.what());
...
int MegaChatApiImpl::performRequest_loadPreview(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            //link format: https://<mega-domain>/chat/<public-handle>#<chat-key>
                        ...
                        mClient->decryptChatTitle(chatId, unifiedKey, encTitle, ph)
                            .then(
                                    ...
                                    std::string decryptedTitle)
                                {
                                    if (wptr.deleted())
                                    {
                                        mMegaApi->sendEvent(99014,
                                                            "karere client instance was removed "
                                                            "upon TYPE_LOAD_PREVIEW",
                                                            false,
                                    ...
                                    ::mega::ChatOptions_t opts =
                                        static_cast<::mega::ChatOptions_t>(result->getParamType());
                                    ...
                                    const MegaScheduledMeetingList* smList =
                                        result->getMegaScheduledMeetingList();
                                    if (smList && smList->size())
                                    {
                                        ...
                                        for (unsigned long i = 0; i < smList->size(); ++i)
                                        {
                                            ...
                                            l->insert(new MegaChatScheduledMeetingPrivate(
                                                new MegaChatScheduledMeetingPrivate(
                                                    *smList->at(i))));
                                        }
                                        request->setMegaChatScheduledMeetingList(l.get());
                                    }
                                    ...
                                    {
                                        GroupChatRoom* room =
                                            dynamic_cast<GroupChatRoom*>(findChatRoom(chatId));
                                        ...
                                        const bool hasChanged =
                                            room && room->hasChatLinkChanged(ph.val,
                                                                             decryptedTitle,
                                                                             meeting,
                                                                             opts);
                                        const std::string url =
                                            result->getLink() ? result->getLink() : "";
                                        const int shard = result->getAccess();
                                        const uint32_t ts =
                                            static_cast<uint32_t>(result->getNumber());
                                        std::shared_ptr<std::string> key =
                                            std::make_shared<std::string>(unifiedKey);
                                        ...
                                        if (room) // chatroom already exists
                                        {
                                            ...
                                            {
                                                // if mcphurl information is different respect
                                                // GroupChatRoom in ram we need to remove preview
                                                // and recreate again, this is simplier than update
                                                ...
                                                mClient->createPublicChatRoom(chatId,
                                                                              ...
                                                                              opts,
                                                                              smList);
                                                room = dynamic_cast<GroupChatRoom*>(
                                                    findChatRoom(chatId));
                                                if (!room)
                                                {
                                                    API_LOG_DEBUG(
                                                    ...
                                                    MegaChatErrorPrivate* megaChatError =
                                                        new MegaChatErrorPrivate(
                                                            MegaChatError::ERROR_UNKNOWN);
                                                    fireOnChatRequestFinish(request, megaChatError);
                                                    return;
                                                }
                                            }
                                            else if (room->previewMode() &&
                                                     !room->isActive()) // re-enable preview
                                            {
                                                API_LOG_DEBUG(
...
int MegaChatApiImpl::performRequest_setPrivateMode(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
...
int MegaChatApiImpl::performRequest_chatLinkHandle(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            MegaChatHandle chatid = request->getChatHandle();
            ...
            {
                API_LOG_DEBUG("%sCannot create chat-links on chatrooms without title",
                              getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }
            ...
            pms.then(
                   [request, wptr, del, room, this](uint64_t ph)
                   {
                       ...
                       {
                           API_LOG_ERROR(
                               "%sUnexpected invalid public handle for query/create chat-link",
                               getLoggingName());
                           return fireOnChatRequestFinish(
                               request,
                               new MegaChatErrorPrivate(MegaChatError::ERROR_NOENT));
                       }

                       room->unifiedKey()
                           .then(
                               [request, this, ph](shared_ptr<string> unifiedKey)
                               {
                                   MegaChatErrorPrivate* megaChatError;
                                   if (!unifiedKey || unifiedKey->size() != 16)
                                   {
                                       API_LOG_ERROR("%sInvalid unified key after decryption",
                                                     getLoggingName());
                                       megaChatError =
                                           new MegaChatErrorPrivate(MegaChatError::ERROR_NOENT);
                                   }
                           ...
                           .fail(
                               [request, this](const ::promise::Error& err)
                               {
                                   API_LOG_ERROR("%sFailed to decrypt unified key: %s",
                                                 getLoggingName(),
                                                 err.what());
...
int MegaChatApiImpl::performRequest_getFirstname(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            mClient->userAttrCache()
                .getAttr(uh, MegaApi::USER_ATTR_FIRSTNAME, ph)
                .then(
                    [request, this](Buffer* data)
                    {
                        MegaChatErrorPrivate* megaChatError =
                            new MegaChatErrorPrivate(MegaChatError::ERROR_OK);
                ...
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sError getting user firstname: %s",
                                      getLoggingName(),
                                      err.what());
...
int MegaChatApiImpl::performRequest_getLastname(MegaChatRequestPrivate *request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            // if the app requested user attributes too early (ie. init with sid but without cache),
            ...
            mClient->userAttrCache()
                .getAttr(uh, MegaApi::USER_ATTR_LASTNAME, ph)
                .then(
                    [request, this](Buffer* data)
                    {
                        MegaChatErrorPrivate* megaChatError =
                ...
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sError getting user lastname: %s",
                                      getLoggingName(),
                                      err.what());
...
int MegaChatApiImpl::performRequest_getEmail(MegaChatRequestPrivate* request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            // if the app requested user attributes too early (ie. init with sid but without cache),
            ...
            MegaChatHandle uh = request->getUserHandle();
            mClient->userAttrCache()
                .getAttr(uh, karere::USER_ATTR_EMAIL)
                .then(
                    [request, this](Buffer* data)
                    {
                        MegaChatErrorPrivate* megaChatError =
                ...
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sError getting user email: %s",
                                      getLoggingName(),
                                      err.what());
...
int MegaChatApiImpl::performRequest_attachNodeMessage(MegaChatRequestPrivate* request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            handle chatid = request->getChatHandle();
            ...
            .fail([this, request, buffer, msgType](const ::promise::Error& err)
            {
                ...
                {
                    API_LOG_WARNING("%sAlready granted access to this node previously",
                                    getLoggingName());
                ...
                else
                {
                    ...
                    API_LOG_ERROR("%sFailed to grant access to some nodes", getLoggingName());
                }
...
int MegaChatApiImpl::performRequest_revokeNodeMessage(MegaChatRequestPrivate* request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            MegaChatHandle chatid = request->getChatHandle();
            ...
            ::promise::when(promise)
                .then(
                    [this, request]()
                    {
                        ...
                        buf.insert(buf.begin(),
                                   Message::kMsgRevokeAttachment - Message::kMsgOffset);
                        buf.insert(buf.begin(), 0x0);

                        std::unique_ptr<MegaChatMessage> megaMsg{
                            sendMessage(request->getChatHandle(), buf.c_str(), buf.length())};
                ...
                .fail(
                    [this, request](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sFailed to revoke access to attached node (%u)",
                                      getLoggingName(),
...
int MegaChatApiImpl::performRequest_setBackgroundStatus(MegaChatRequestPrivate* request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
...
int MegaChatApiImpl::performRequest_pushReceived(MegaChatRequestPrivate* request)
{
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            MegaChatHandle chatid = request->getChatHandle();
                ...
                .then(
                    [this, request, wptr, wasArchived, iosPushReceived]()
                    {
                        ...
                        if (!iosPushReceived) // Android
                        {
                            ...
                            // received a push, so the previous notifications are cleanup and the
                            // new set of messages are notified

                            MegaHandleList* chatids = MegaHandleList::createInstance();

                            // for each chatroom, load all unread messages)
                            for (auto it = mClient->chats->begin(); it != mClient->chats->end();
                                 it++)
                            {
                                MegaChatHandle chatid = it->first;
                                // don't want to generate notifications for archived chats or chats
                                ...
                                // first msg to consider: last-seen if loaded in memory. Otherwise,
                                // the oldest loaded msg
                            ...
                            request->setMegaHandleList(
                                chatids); // always a valid list, even if empty
                            delete chatids;
                        }
                        else // iOS
                        {
                            ...
                            if (!room)
                            {
                                mMegaApi->sendEvent(99006,
                                                    "iOS PUSH received for non-existing chatid",
                                                    false,
                                                    static_cast<const char*>(nullptr));
                                ...
                                return;
                                     ...
                                     room->isArchived()) // don't want to generate notifications for
                                                         // archived chats
                            {
                                mMegaApi->sendEvent(
                                    99009,
                                    "PUSH received for archived chatid (and still archived)",
                                    false,
                                    static_cast<const char*>(nullptr));

                                // since a PUSH could be received before the actionpacket updating
                                // flags (
                                MegaChatErrorPrivate* megaChatError =
                                    new MegaChatErrorPrivate(MegaChatError::ERROR_ACCESS);
                                fireOnChatRequestFinish(request, megaChatError);
                                return;
                            }
                        }
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            if (!mClient->rtc)
            {
                API_LOG_ERROR("%sStart call - WebRTC is not initialized", getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }
            ...
            if (!chatroom)
            {
                API_LOG_ERROR("%sStart call - Chatroom has not been found", getLoggingName());
                return MegaChatError::ERROR_NOENT;
            }

            if (!isValidSimVideoTracks(mClient->rtc->getNumInputVideoTracks()))
            {
                API_LOG_ERROR("%sStart call - Invalid value for simultaneous input video tracks",
                              getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            if (!chatroom->isGroup())
            {
                ...
                {
                    API_LOG_ERROR("%sStart call - Refusing start a call with a non active contact",
                                  getLoggingName());
                    return MegaChatError::ERROR_ACCESS;
                }
            }
            ...
            {
                API_LOG_ERROR("%sStart call - Refusing start a call withouth enough privileges",
                              getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }
            ...
            {
                API_LOG_ERROR("%sStart call - Chatroom is in preview mode", getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }

            if (chatroom->chatdOnlineState() != kChatStateOnline)
            {
                API_LOG_ERROR("%sStart call - chatroom isn't in online state", getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }
            ...
            if (!call)
            {
               ...
               {
                   API_LOG_ERROR("%sStart call - start call attempt already in progress",
                                 getLoggingName());
                   return MegaChatError::ERROR_EXIST;
               }
                          ...
                          shared_ptr<string> unifiedKey)
                      {
                          ...
                          mClient->rtc->startCall(chatid, avFlags, isGroup, notRinging, unifiedKey)
                              ...
                              .fail(
                                  [request, this](const ::promise::Error& err)
                                  {
                                      API_LOG_ERROR("%sError Starting a chat call: %s",
                                                    getLoggingName(),
                                                    err.what());

                                      MegaChatErrorPrivate* megaChatError =
                                          new MegaChatErrorPrivate(err.msg(),
                                                                   err.code(),
                                                                   err.type());
                                      fireOnChatRequestFinish(request, megaChatError);
                                  });
            ...
            else if (!call->participate())
            {
                if (call->isJoining())
                {
                    API_LOG_ERROR("%sStart call - joining call attempt already in progress",
                                  getLoggingName());
                ...
                call->join(avFlags)
                    ...
                    .fail(
                        [request, this](const ::promise::Error& err)
                        {
                            API_LOG_ERROR("%sError Joining a chat call: %s",
                                          getLoggingName(),
                                          err.what());
            ...
            else
            {
                ...
                API_LOG_ERROR("%sA call exists in this chatroom and we already participate or it's "
                              "not a groupchat",
                              getLoggingName());
                request->setUserHandle(call->getCallid());
                return MegaChatError::ERROR_EXIST;
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            if (!chatroom)
            {
                API_LOG_ERROR("%sAnswer call - Chatroom has not been found", getLoggingName());
                return MegaChatError::ERROR_NOENT;
            }
            ...
            {
                API_LOG_ERROR(
                    "%sAnswer call - Refusing answer a call, clientid no yet assigned by shard: %d",
                    getLoggingName(),
                    chatroom->chat().connection().shardNo());
                return MegaChatError::ERROR_ACCESS;
            }

            if (chatroom->chatdOnlineState() != kChatStateOnline)
            {
                API_LOG_ERROR("%sAnswer call - chatroom isn't in online state", getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }

            if (!isValidSimVideoTracks(mClient->rtc->getNumInputVideoTracks()))
            {
                API_LOG_ERROR("%sAnswer call - Invalid value for simultaneous input video tracks",
                              getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            if (!call)
            {
                API_LOG_ERROR("%sAnswer call - There is not any call in that chatroom",
                              getLoggingName());
                return MegaChatError::ERROR_NOENT;
            }
            ...
            if (call->participate())
            {
                API_LOG_ERROR("%sAnswer call - You already participate", getLoggingName());
                return MegaChatError::ERROR_EXIST;
            }

            if (call->isJoining())
            {
                API_LOG_ERROR("%sAnswer call - joining call attempt already in progress",
                              getLoggingName());
                return MegaChatError::ERROR_EXIST;
            }
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            if (!mClient->rtc)
            {
                API_LOG_ERROR("%sHang up call - WebRTC is not initialized", getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }
            ...
            {
                API_LOG_ERROR("%sHang up call - invalid callid", getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            if (!call)
            {
                API_LOG_ERROR("%sHang up call - There is not any call with that callid",
                              getLoggingName());
                return MegaChatError::ERROR_NOENT;
            }
            ...
            if (!chatroom)
            {
                API_LOG_ERROR("%sHang up call- Chatroom has not been found", getLoggingName());
                return MegaChatError::ERROR_NOENT;
            }
                    ...
                    && (static_cast<int>(chatroom->ownPriv()) != static_cast<int>(MegaChatPeerList::PRIV_MODERATOR)))
            {
                ...
                API_LOG_ERROR("%sEnd call withouth enough privileges", getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }
            ...
            pms.then(
                ...
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sError hang up a chat call: %s",
                                      getLoggingName(),
                                      err.what());
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            if (!call)
            {
                API_LOG_ERROR("%sDisable AV flags  - There is not any call in that chatroom",
                              getLoggingName());
                assert(call);
                return MegaChatError::ERROR_NOENT;
            }
            ...
            if (operationType == MegaChatRequest::AUDIO)
            {
                if (enable)
                {
                    ...
                    {
                        API_LOG_WARNING(
                            "%sEnable audio - isn't allow, peer doesn't have speak permission",
                            getLoggingName());
                        return MegaChatError::ERROR_ACCESS;
                    }
                }
            ...
            if (!call->participate())
            {
                API_LOG_ERROR("%sDisable audio video - You don't participate in the call",
                              getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            if (!call)
            {
                API_LOG_ERROR("%sSet call on hold  - There is not any call in that chatroom",
                              getLoggingName());
                assert(call);
                return MegaChatError::ERROR_NOENT;
            }
            ...
            {
                API_LOG_ERROR("%sThe call can't be set onHold until call is in-progres",
                              getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }
            ...
            {
                API_LOG_ERROR(
                    "%sSet call on hold - Call is on hold and try to set on hold or conversely",
                    getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
...
int MegaChatApiImpl::performRequest_setVideoCapturerInDevice(MegaChatRequestPrivate* request)
{
    if (!mClient->rtc)
    {
        API_LOG_ERROR("%sChange video streaming source - WebRTC is not initialized",
                      getLoggingName());
        return MegaChatError::ERROR_ACCESS;
    }
    ...
    {
        API_LOG_ERROR("%sChange video streaming source - device doesn't exist", getLoggingName());
        return MegaChatError::ERROR_ARGS;
    }
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            chatroom->archiveChat(archive)
                ...
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sError archiving chat: %s", getLoggingName(), err.what());
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            for (unsigned int i = 0; i < handleList->size(); i++)
            {
                ...
                {
                    API_LOG_ERROR("%sError %s is not a chat member of chatroom (%s)",
                                  getLoggingName(),
                                  Id(peerid).toString().c_str(),
                                  Id(chatid).toString().c_str());
                    isMember = false;
                    break;
                }
            }
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            if (chatid == MEGACHAT_INVALID_HANDLE)
            {
                API_LOG_ERROR("%sRequest(TYPE_SET_RETENTION_TIME). Invalid chatid",
                              getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            if (!chatroom)
            {
                API_LOG_ERROR("%sRequest(TYPE_SET_RETENTION_TIME). Chatroom not found. Chatid: %s",
                              getLoggingName(),
                              karere::Id(chatid).toString().c_str());
                return MegaChatError::ERROR_NOENT;
            }
            ...
            {
                API_LOG_ERROR(
                    "%sRequest(TYPE_SET_RETENTION_TIME). Insufficient permissions for chatroom: %s",
                    getLoggingName(),
                    karere::Id(chatid).toString().c_str());
                return MegaChatError::ERROR_ACCESS;
            }
            ...
            chatroom->setChatRetentionTime(period)
                ...
                .fail(
                    ...
                    {
                        API_LOG_ERROR(
                            "%sRequest(TYPE_SET_RETENTION_TIME). Error setting retention time. %s",
                            getLoggingName(),
                            err.what());
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            if (chatid == MEGACHAT_INVALID_HANDLE)
            {
                API_LOG_ERROR("%sMegaChatRequest::TYPE_ENABLE_AUDIO_LEVEL_MONITOR - Invalid chatid",
                              getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            {
                API_LOG_ERROR(
                    "%sEnable audio level monitor  - There is not any call in that chatroom",
                    getLoggingName());
                assert(call);
                return MegaChatError::ERROR_NOENT;
            }
            ...
            if (!call->participate())
            {
                API_LOG_ERROR("%sEnable audio level monitor - You don't participate in the call",
                              getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }
...
int MegaChatApiImpl::performRequest_addDelspeakRequest(MegaChatRequestPrivate* request)
{
    ...
    if (chatid == MEGACHAT_INVALID_HANDLE)
    {
        API_LOG_ERROR("%s%s - Invalid chatid", getLoggingName(), errMsg.c_str());
        return MegaChatError::ERROR_ARGS;
    }

    if (add && user != MEGACHAT_INVALID_HANDLE)
    {
        API_LOG_ERROR("%s%s - expected userid for adding speak request",
                      getLoggingName(),
                      errMsg.c_str());
        return MegaChatError::ERROR_ARGS;
    }
    ...
    {
        API_LOG_ERROR("%s%s - can't get chat call", getLoggingName(), errMsg.c_str());
        return res.first;
    }
    ...
    {
        API_LOG_ERROR("%s - userid doesn't has a pending speak request in flight", errMsg.c_str());
        return MegaChatError::ERROR_NOENT;
    }

    if (add && (call->hasUserSpeakPermission(getMyUserHandle()) || call->hasUserPendingSpeakRequest(getMyUserHandle())))
    {
        API_LOG_ERROR("%s%s - userid already has ",
                      getLoggingName(),
                      call->hasUserSpeakPermission(getMyUserHandle()) ?
                          " speak permission" :
...
{
        {
            ...
            if (add && user == MEGACHAT_INVALID_HANDLE)
            {
                API_LOG_ERROR("%s%s - Invalid userid", getLoggingName(), errMsg.c_str());
                return MegaChatError::ERROR_ARGS;
            }
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            if (chatid == MEGACHAT_INVALID_HANDLE)
            {
                API_LOG_ERROR("%sMegaChatRequest::TYPE_REQUEST_HIGH_RES_VIDEO - Invalid chatid",
                              getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            {
                API_LOG_ERROR(
                    "%sMegaChatRequest::TYPE_REQUEST_HIGH_RES_VIDEO  - Invalid resolution quality",
                    getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            if (!call)
            {
                API_LOG_ERROR("%sMegaChatRequest::TYPE_REQUEST_HIGH_RES_VIDEO  - There is not any "
                              "call in that chatroom",
                              getLoggingName());
                assert(call);
                return MegaChatError::ERROR_NOENT;
            }
            ...
            {
                API_LOG_ERROR("%sMegaChatRequest::TYPE_REQUEST_HIGH_RES_VIDEO - Invalid list of "
                              "Cids for removal",
                              getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            {
                API_LOG_ERROR("%sRequest high resolution video - Call isn't in progress state",
                              getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            if (chatid == MEGACHAT_INVALID_HANDLE)
            {
                API_LOG_ERROR("%sMegaChatRequest::TYPE_REQUEST_LOW_RES_VIDEO - Invalid chatid",
                              getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            if (!call)
            {
                API_LOG_ERROR("%sMegaChatRequest::TYPE_REQUEST_LOW_RES_VIDEO  - There is not any "
                              "call in that chatroom",
                              getLoggingName());
                assert(call);
                return MegaChatError::ERROR_NOENT;
            ...
            {
                API_LOG_ERROR(
                    "%sMegaChatRequest::TYPE_REQUEST_LOW_RES_VIDEO - Call isn't in progress state",
                    getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }
            ...
            {
                API_LOG_ERROR(
                    "%sMegaChatRequest::TYPE_REQUEST_LOW_RES_VIDEO - Invalid list of Cids",
                    getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
...
int MegaChatApiImpl::performRequest_openCloseVideoDevice(MegaChatRequestPrivate* request)
{
    if (!mClient->rtc)
    {
        API_LOG_ERROR("%sOpenVideoDevice - WebRTC is not initialized", getLoggingName());
        return MegaChatError::ERROR_ACCESS;
    }
    ...
    {
        API_LOG_ERROR("%sOpenVideoDevice - invalid capturer type", getLoggingName());
        return MegaChatError::ERROR_ARGS;
    }
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            if (chatid == MEGACHAT_INVALID_HANDLE)
            {
                API_LOG_ERROR("%sMegaChatRequest::TYPE_REQUEST_HIRES_QUALITY - Invalid chatid",
                              getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            if (!call)
            {
                API_LOG_ERROR("%sMegaChatRequest::TYPE_REQUEST_HIRES_QUALITY  - There is not any "
                              "call in that chatroom",
                              getLoggingName());
                assert(call);
                return MegaChatError::ERROR_NOENT;
            ...
            {
                API_LOG_ERROR(
                    "%sMegaChatRequest::TYPE_REQUEST_HIRES_QUALITY - Call isn't in progress state",
                    getLoggingName());
                return MegaChatError::ERROR_ACCESS;
            }
            ...
            {
                API_LOG_ERROR("%sMegaChatRequest::TYPE_REQUEST_HIRES_QUALITY  - Currently not "
                              "receiving a hi-res stream for this peer",
                              getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            {
                API_LOG_ERROR("%sMegaChatRequest::TYPE_REQUEST_HIRES_QUALITY  - invalid quality "
                              "level value (spatial layer offset).",
                              getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
...
int MegaChatApiImpl::performRequest_pushOrAllowJoinCall(MegaChatRequestPrivate* request)
{
    ...
    if (chatid == MEGACHAT_INVALID_HANDLE)
    {
        API_LOG_ERROR("%s%s - Invalid chatid: %s",
                      getLoggingName(),
                      reqStr.c_str(),
                      chatidB64.c_str());
        return MegaChatError::ERROR_ARGS;
    ...
    {
        API_LOG_ERROR("%s%s - Invalid list of users provided. chatid: %s",
                      getLoggingName(),
                      reqStr.c_str(),
                      chatidB64.c_str());
        return MegaChatError::ERROR_ARGS;
    ...
    if (!call)
    {
        API_LOG_ERROR("%s%s - There is not any call in that chatroom. chatid: %s",
                      getLoggingName(),
                      reqStr.c_str(),
                      chatidB64.c_str());
        assert(call);
    ...
    if (call->getState() != rtcModule::kStateInProgress)
    {
        API_LOG_ERROR("%s%s - Call isn't in progress state. chatid: %s",
                      getLoggingName(),
                      reqStr.c_str(),
                      chatidB64.c_str());
        return MegaChatError::ERROR_ACCESS;
    ...
    if (!chatroom)
    {
        API_LOG_ERROR("%s%s - Chatroom has not been found. chatid: %s",
                      getLoggingName(),
                      reqStr.c_str(),
                      chatidB64.c_str());
        return MegaChatError::ERROR_NOENT;
    ...
    {
        API_LOG_ERROR("%s%s - Chatroom doesn't have waiting room enabled. chatid: %s",
                      getLoggingName(),
                      reqStr.c_str(),
                      chatidB64.c_str());
        return MegaChatError::ERROR_NOENT;
    ...
    {
        API_LOG_ERROR("%s%s - Insufficient permissions to execute this action. chatid: %s",
                      getLoggingName(),
                      reqStr.c_str(),
                      chatidB64.c_str());
        return MegaChatError::ERROR_ACCESS;
    ...
    if (!waitingRoom)
    {
        ...
        API_LOG_ERROR("%s%s. Can't retrieve waiting room from karere chatroom. chatid: %s",
                      getLoggingName(),
                      reqStr.c_str(),
                      chatidB64.c_str());
        assert(false);
...
int MegaChatApiImpl::performRequest_kickUsersFromCall(MegaChatRequestPrivate* request)
{
    ...
    {
        API_LOG_ERROR("%sMegaChatRequest::TYPE_WR_KICK - Empty list of users to be kicked off",
                      getLoggingName());
        return MegaChatError::ERROR_ARGS;
    }
    ...
    if (!waitingRoom)
    {
        ...
        // however we also need to ensure, that we also have received proper information from SFU to
        // initialize waiting room with users on it, and their joining permission
        API_LOG_ERROR("%sMegaChatRequest::TYPE_WR_KICK. Can't retrieve waiting room from karere "
                      "chatroom. chatid: %s",
                      getLoggingName(),
...
int MegaChatApiImpl::performRequest_setLimitsInCall(MegaChatRequestPrivate* request)
{
    ...
    if (chatid == MEGACHAT_INVALID_HANDLE)
    {
        API_LOG_ERROR("%sMegaChatRequest::TYPE_SET_LIMIT_CALL - Invalid chatid", getLoggingName());
        return MegaChatError::ERROR_ARGS;
    }

    MegaHandleList* handleList = request->getMegaHandleList();
    if (!handleList || handleList->size() != 5)
    {
        return MegaChatError::ERROR_ARGS;
    }
    ...
    {
        API_LOG_ERROR(
            "%sMegaChatRequest::TYPE_SET_LIMIT_CALL - There is not any call in that chatroom",
            getLoggingName());
        assert(call);
        return MegaChatError::ERROR_NOENT;
    }
    ...
    if (call->getState() != rtcModule::kStateInProgress)
    {
        API_LOG_ERROR("%sMegaChatRequest::TYPE_SET_LIMIT_CALL - Call isn't in progress state",
                      getLoggingName());
        return MegaChatError::ERROR_ACCESS;
    }
    ...
    const uint32_t divider = static_cast<uint32_t>(handleList->get(4));
    ...
    if (callDurSecs == MegaChatCall::CALL_LIMIT_NO_PRESENT
        ...
        && divider == MegaChatCall::CALL_LIMIT_NO_PRESENT)
    {
        API_LOG_ERROR("%sMegaChatRequest::TYPE_SET_LIMIT_CALL - invalid value for provided params",
                      getLoggingName());
        return MegaChatError::ERROR_ARGS;
    }
...
int MegaChatApiImpl::performRequest_mutePeersInCall(MegaChatRequestPrivate* request)
{
    ...
    if (chatid == MEGACHAT_INVALID_HANDLE)
    {
        API_LOG_ERROR("%sMegaChatRequest::TYPE_MUTE - Invalid chatid", getLoggingName());
        return MegaChatError::ERROR_ARGS;
    }
    ...
    if (!call)
    {
        API_LOG_ERROR("%sMegaChatRequest::TYPE_MUTE - There is not any call in that chatroom",
                      getLoggingName());
        assert(call);
        return MegaChatError::ERROR_NOENT;
    }
    ...
    if (call->getState() != rtcModule::kStateInProgress)
    {
        API_LOG_ERROR("%sMegaChatRequest::TYPE_MUTE - Call isn't in progress state",
                      getLoggingName());
        return MegaChatError::ERROR_ACCESS;
    }
    ...
    {
        API_LOG_ERROR(
            "%sMegaChatRequest::TYPE_MUTE - moderator role required to perform this action",
            getLoggingName());
        return MegaChatError::ERROR_ACCESS;
    }
    ...
    {
        API_LOG_ERROR("%sMegaChatRequest::TYPE_MUTE - Session with Cid not exists %d. Callid: %s",
                      getLoggingName(),
                      cid,
                      karere::Id(call->getCallid()).toString().c_str());
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            if (chatid == MEGACHAT_INVALID_HANDLE)
            {
                API_LOG_ERROR("%sMegaChatRequest::TYPE_DELETE_SCHEDULED_MEETING - Invalid chatid",
                              getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            {
                API_LOG_ERROR("%sMegaChatRequest::TYPE_DELETE_SCHEDULED_MEETING - Invalid schedId",
                              getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            mClient->removeScheduledMeeting(chatid, schedId)
                ...
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sError removing a scheduled meeting: %s",
                                      getLoggingName(),
                                      err.what());
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            {
                API_LOG_ERROR(
                    "%sMegaChatRequest::TYPE_FETCH_SCHEDULED_MEETING_OCCURRENCES - Invalid chatid",
                    getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            if (res.size() < numOccurrences) // fetch fresh occurrences from API
            {
                ...
                API_LOG_DEBUG("%sFetching fresh scheduled meeting occurrences from API",
                              getLoggingName());
                mClient
                    ->fetchScheduledMeetingOccurrences(chatid, newSince, until, pendingOccurrences)
                    .then(
                        [request, l, this](
                            std::vector<std::shared_ptr<KarereScheduledMeetingOccurr>> result)
                        {
                            std::for_each(
                                result.begin(),
                                result.end(),
                    ...
                    .fail(
                        [request, this](const ::promise::Error& err)
                        {
                            API_LOG_ERROR("%sError fetching scheduled meetings occurrences: %s",
                                          getLoggingName(),
                                          err.what());
                            std::unique_ptr<MegaChatScheduledMeetingOccurrList> list(
                                MegaChatScheduledMeetingOccurrList::createInstance());
            ...
            {
                API_LOG_DEBUG("%sScheduled meeting occurrences fetched from local",
                              getLoggingName());
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            {
                API_LOG_ERROR(
                    "%sTYPE_UPDATE_SCHEDULED_MEETING_OCCURRENCE error 1 (list %s, size %ul): %d",
                    getLoggingName(),
            ...
            if (ocurr->startDateTime() == MEGACHAT_INVALID_TIMESTAMP &&
                    ...
                    ocurr->cancelled() == -1)
            {
                API_LOG_ERROR("%sTYPE_UPDATE_SCHEDULED_MEETING_OCCURRENCE error 2 (bad "
                              "startDateTime, endDateTime, cancelled): %d",
                              getLoggingName(),
                              MegaChatError::ERROR_NOENT);
                return MegaChatError::ERROR_NOENT;
            }
            ...
            if (!chatroom)
            {
                API_LOG_ERROR("%sTYPE_UPDATE_SCHEDULED_MEETING_OCCURRENCE error 3 (chatroom not "
                              "found for id %s): %d",
                              getLoggingName(),
                              Base64Str<MegaClient::CHATHANDLE>(ocurr->chatId()).chars,
                              MegaChatError::ERROR_NOENT);
                return MegaChatError::ERROR_NOENT;
            }
            ...
            if (it == schedMeetings.end())
            {
                ...
                API_LOG_ERROR("%sTYPE_UPDATE_SCHEDULED_MEETING_OCCURRENCE error 4 (scheduled "
                              "meeting not found for id %s): %d",
                              getLoggingName(),
                              Base64Str<MegaClient::CHATHANDLE>(ocurr->schedId()).chars,
                              MegaChatError::ERROR_NOENT);
                return MegaChatError::ERROR_NOENT;
            }
            ...
            if (!localOccurr) // occurrence could not be found in RAM
            {
                ...
                if (res.size() < MegaChatScheduledMeeting::MIN_OCURRENCES)
                {
                    ...
                    mClient
                        ...
                        .then(
                            [this](std::vector<
                                   std::shared_ptr<KarereScheduledMeetingOccurr>> /*result*/)
                            {
                                API_LOG_DEBUG(
                                    "%sNewer scheduled meetings occurrences retrieved successfully",
                                    getLoggingName());
                            })
                        .fail(
                    ...
                    API_LOG_ERROR("%sTYPE_UPDATE_SCHEDULED_MEETING_OCCURRENCE error 6 "
                                  "(scheduled meeting occurrence not found in local for id %s and "
                                  "overrides %d): %d",
                                  getLoggingName(),
                                  Base64Str<MegaClient::CHATHANDLE>(ocurr->schedId()).chars,
                                  overrides,
                ...
                API_LOG_ERROR(
                    "%sTYPE_UPDATE_SCHEDULED_MEETING_OCCURRENCE error 5 "
            ...
            mClient->createOrUpdateScheduledMeeting(megaSchedMeeting.get(), nullptr /*chatTitle*/)
                .then(
                    [request, this](KarereScheduledMeeting* sm)
                    {
                        if (sm)
                        {
                            std::unique_ptr<MegaChatScheduledMeetingList> l(
                        ...
                        {
                            assert(false);
                            API_LOG_ERROR("%sError updating a scheduled meeting occurrence, "
                                          "updated occurrence has not been received",
                                          getLoggingName());
                            fireOnChatRequestFinish(
                                request,
                                new MegaChatErrorPrivate(MegaChatError::ERROR_UNKNOWN));
                ...
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sError updating a scheduled meeting occurrence: %s",
                                      getLoggingName(),
                                      err.what());
    ...
    // keep indent and dummy scope from original code in sendPendingRequests(), for a smaller diff
        {
            ...
            {
                API_LOG_ERROR(
                    "%sError updating a scheduled meeting: title or description length exceeded",
                    getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            {
                API_LOG_ERROR("%sError updating a scheduled meeting: updateChatTitle is true but "
                              "scheduled meeting title has not changed",
                              getLoggingName());
                return MegaChatError::ERROR_ARGS;
            }
            ...
            auto wptr = mClient->weakHandle();
            pms.then(
                   [this, wptr, request, megaSchedMeeting, &updateChatTitle](
                       const std::shared_ptr<Buffer>& buf)
                   {
                       ...
                       {
                           MegaChatErrorPrivate* megaChatError =
                               new MegaChatErrorPrivate(MegaChatError::ERROR_TOOMANY);
                           fireOnChatRequestFinish(request, megaChatError);
                           return;
                       }
                       ...
                       mClient->createOrUpdateScheduledMeeting(megaSchedMeeting.get(), chatTitle)
                           .then(
                               [request, this](KarereScheduledMeeting* sm)
                               {
                                   ...
                                   {
                                       assert(false);
                                       API_LOG_ERROR("%sError updating a scheduled meeting, non "
                                                     "received scheduled meeting",
                                                     getLoggingName());
                                       fireOnChatRequestFinish(
                                           request,
                                           new MegaChatErrorPrivate(MegaChatError::ERROR_UNKNOWN));
                           ...
                           .fail(
                               [request, this](const ::promise::Error& err)
                               {
                                   API_LOG_ERROR("%sError updating a scheduled meeting: %s",
                                                 getLoggingName(),
                                                 err.what());

                                   MegaChatErrorPrivate* megaChatError =
                                       new MegaChatErrorPrivate(err.code());
                                   fireOnChatRequestFinish(request, megaChatError);
                               });
                   })
                .fail(
                    [request, this](const ::promise::Error& err)
                    {
                        API_LOG_ERROR("%sError encrypting chatroom title for all participants: %s",
                                      getLoggingName(),
                                      err.what());
                        MegaChatErrorPrivate* megaChatError = new MegaChatErrorPrivate(err.code());
                        fireOnChatRequestFinish(request, megaChatError);
...
#endif
        API_LOG_DEBUG("%screateKarereClient: karere client instance created", getLoggingName());
        mTerminating = false;
    }
}
...
void MegaChatApiImpl::removeChatRoomHandler(MegaChatHandle chatid)
{
    ...
    if (it == chatRoomHandler.end())
    {
        API_LOG_WARNING("%sremoveChatRoomHandler: chatroom handler not found (chatid: %s)",
                        getLoggingName(),
                        ID_CSTR(chatid));
        return;
    }
...
karere::ChatRoom *MegaChatApiImpl::findChatRoomByUser(MegaChatHandle userhandle)
{
    ...
    if (userhandle == mClient->myHandle().val)
    {
        return mClient->chats->selfChat();
    }
    ContactList::iterator it = mClient->mContactList->find(userhandle);
    if (it != mClient->mContactList->end())
    {
        return it->second->chatRoom();
    }
    return nullptr;
}
...
bool MegaChatApiImpl::openNodeHistory(MegaChatHandle chatid, MegaChatNodeHistoryListener *listener)
{
    ...
    if (chatroom)
    {
        ...
        {
            sdkMutex.unlock();
            API_LOG_WARNING("%sopenNodeHistory: node history is already open for this chatroom "
                            "(chatid: %s), close it before open it again",
                            getLoggingName(),
                            karere::Id(chatid).toString().c_str());
...
void MegaChatApiImpl::addNodeHistoryListener(MegaChatHandle chatid, MegaChatNodeHistoryListener *listener)
{
    ...
    {
        assert(false);
        API_LOG_WARNING("%saddNodeHistoryListener: node history handler not found (chatid: %s)",
                        getLoggingName(),
                        karere::Id(chatid).toString().c_str());
    }
...
void MegaChatApiImpl::removeNodeHistoryListener(MegaChatHandle chatid, MegaChatNodeHistoryListener *listener)
{
    ...
    {
        assert(false);
        API_LOG_WARNING("%sremoveNodeHistoryListener: node history handler not found (chatid: %s)",
                        getLoggingName(),
                        karere::Id(chatid).toString().c_str());
    }
...
{
    API_LOG_INFO("%sRequest (%s) starting", getLoggingName(), request->getRequestString());
...
void MegaChatApiImpl::fireOnChatRequestFinish(MegaChatRequestPrivate *request, MegaChatError *e)
{
    if(e->getErrorCode())
    {
        API_LOG_INFO("%sRequest (%s) finished with error: %s",
                     getLoggingName(),
    ...
    {
        API_LOG_INFO("%sRequest (%s) finished", getLoggingName(), request->getRequestString());
    }
...
void MegaChatApiImpl::fireOnChatVideoData(MegaChatHandle chatid, uint32_t clientId, int width, int height, int sourceType, char *buffer, rtcModule::VideoResolution videoResolution)
{
    ...
    if (clientId == 0)
    {
        ...
        {
            API_LOG_ERROR("%sfireOnChatVideoData. Invalid sourceType: %d",
                          getLoggingName(),
                          sourceType);
            assert(false);
            return;
    ...
    if (it != itEnd)
    {
        ...
        if (peerVideoIterator != it->second.end())
        {
                 ...
                 videoListenerIterator++)
            {
                ...
                {
                    API_LOG_WARNING("%sremote videoListener with CID %u does not exists ",
                                    getLoggingName(),
                                    clientId);
                    continue;
                }
...
void MegaChatApiImpl::retryPendingConnections(bool disconnect, bool refreshURL, MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_retryPendingConnections(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::logout(MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_logout(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::setOnlineStatus(int status, MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_setOnlineStatus(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::setPresenceAutoaway(bool enable, int64_t timeout, MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_setPresenceAutoaway(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::setPresencePersist(bool enable, MegaChatRequestListener *listener)
{
    ...
    request->setFlag(enable);
    request->setPerformRequest([this, request]() { return performRequest_setPresencePersist(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::signalPresenceActivity(MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_signalActivity(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::setLastGreenVisible(bool enable, MegaChatRequestListener *listener)
{
    ...
    request->setFlag(enable);
    request->setPerformRequest([this, request]() { return performRequest_setLastGreenVisible(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::requestLastGreen(MegaChatHandle userid, MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_lastGreen(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::setBackgroundStatus(bool background, MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_setBackgroundStatus(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::getUserFirstname(MegaChatHandle userhandle, const char *authorizationToken, MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_getFirstname(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::getUserLastname(MegaChatHandle userhandle, const char *authorizationToken, MegaChatRequestListener *listener)
{
    ...
    request->setUserHandle(userhandle);
    request->setLink(authorizationToken);
    request->setPerformRequest([this, request]() { return performRequest_getLastname(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::getUserEmail(MegaChatHandle userhandle, MegaChatRequestListener *listener)
{
    ...
    request->setUserHandle(userhandle);
    request->setPerformRequest([this, request]() { return performRequest_getEmail(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::setChatOption(MegaChatHandle chatid, int option, bool enabled, MegaChatRequestListener* listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_setChatroomOptions(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::createChat(bool group, MegaChatPeerList *peerList, MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_createChatroom(request); });
    requestQueue.push(request);
    waiter->notify();
}
    ...
    for (auto it = mClient->chats->begin(); it != mClient->chats->end(); it++)
    {
       ...
       if (chatRoom)
       {
            ...
            for (auto it = map.begin(); it != map.end(); it++)
            {
                if (!it->second)
                {
                    API_LOG_ERROR("%sgetAllScheduledMeetings: scheduled meeting NULL at scheduled "
                                  "meeting list",
                                  getLoggingName());
                    assert(false);
                    continue;
                ...
                {
                    API_LOG_ERROR(
                        "%sgetAllScheduledMeetings: scheduled meeting should have a timezone",
                        getLoggingName());
                    assert(false);
                    continue;
                }
...
void MegaChatApiImpl::chatLinkHandle(MegaChatHandle chatid, bool del, bool createifmissing, MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_chatLinkHandle(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::inviteToChat(MegaChatHandle chatid, MegaChatHandle uh, int privilege, MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_inviteToChatroom(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::autojoinPublicChat(MegaChatHandle chatid, MegaChatRequestListener *listener)
{
    ...
    request->setChatHandle(chatid);
    request->setPerformRequest([this, request]() { return performRequest_autojoinPublicChat(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::removeFromChat(MegaChatHandle chatid, MegaChatHandle uh, MegaChatRequestListener *listener)
{
    ...
    request->setChatHandle(chatid);
    request->setUserHandle(uh);
    request->setPerformRequest([this, request]() { return performRequest_removeFromChatroom(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::updateChatPermissions(MegaChatHandle chatid, MegaChatHandle uh, int privilege, MegaChatRequestListener *listener)
{
    ...
    request->setUserHandle(uh);
    request->setPrivilege(privilege);
    request->setPerformRequest([this, request]() { return performRequest_updatePeerPermissions(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::truncateChat(MegaChatHandle chatid, MegaChatHandle messageid, MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_truncateHistory(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::setChatTitle(MegaChatHandle chatid, const char *title, MegaChatRequestListener *listener)
{
    ...
    request->setChatHandle(chatid);
    request->setText(title);
    request->setPerformRequest([this, request]() { return performRequest_editChatroomName(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::openChatPreview(const char *link, MegaChatRequestListener *listener)
{
    ...
    request->setFlag(true);
    request->setPerformRequest([this, request]() { return performRequest_loadPreview(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::setPublicChatToPrivate(MegaChatHandle chatid, MegaChatRequestListener *listener)
{
    ...
    request->setChatHandle(chatid);
    request->setPerformRequest([this, request]() { return performRequest_setPrivateMode(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
bool MegaChatApiImpl::openChatRoom(MegaChatHandle chatid, MegaChatRoomListener *listener)
{
    ...
    if (!chatroom)
    {
        API_LOG_WARNING("%sopenChatRoom. chatroom not found. chatid: %s",
                        getLoggingName(),
                        karere::Id(chatid).toString().c_str());
        return false;
    }
    ...
    {
        API_LOG_WARNING("%sApp chat handler is already set, remove it first. chatid: %s",
                        getLoggingName(),
                        karere::Id(chatid).toString().c_str());
        return false;
    }
...
void MegaChatApiImpl::closeChatRoom(MegaChatHandle chatid, MegaChatRoomListener *listener)
{
    ...
    {
        API_LOG_WARNING("%scloseChatRoom(): chatid not found [%s]",
                        getLoggingName(),
                        karere::Id(chatid).toString().c_str());
    }
...
MegaChatMessage *MegaChatApiImpl::getMessage(MegaChatHandle chatid, MegaChatHandle msgid)
{
    ...
    if (chatroom)
    {
        ...
        if (index != CHATD_IDX_INVALID)     // only confirmed messages have index
        {
            ...
            {
                API_LOG_ERROR("%sFailed to find message by index, being index retrieved from "
        ...
        else    // message still not confirmed, search in sending-queue
        {
            ...
            {
                API_LOG_ERROR("%sFailed to find message by temporal id (id: %s)",
                              getLoggingName(),
                              ID_CSTR(msgid));
            }
        }
    ...
    {
        API_LOG_ERROR("%sChatroom not found (chatid: %s)", getLoggingName(), ID_CSTR(chatid));
    }
...
MegaChatMessage *MegaChatApiImpl::getMessageFromNodeHistory(MegaChatHandle chatid, MegaChatHandle msgid)
{
    ...
    if (chatroom)
    {
        ...
        {
            API_LOG_ERROR("%sFailed to find message at node history (id: %s)",
                          getLoggingName(),
                          ID_CSTR(msgid));
        }
    }
...
MegaChatMessage *MegaChatApiImpl::getManualSendingMessage(MegaChatHandle chatid, MegaChatHandle rowid)
{
    ...
    if (chatroom)
    {
        ...
        {
            API_LOG_ERROR("%sMessage not found (rowid: %lu)", getLoggingName(), rowid);
        }
    }
    else
    {
...
void MegaChatApiImpl::attachNodes(MegaChatHandle chatid, MegaNodeList *nodes, MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_attachNodeMessage(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::revokeAttachment(MegaChatHandle chatid, MegaChatHandle handle, MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_revokeNodeMessage(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
void MegaChatApiImpl::pushReceived(bool beep, MegaChatHandle chatid, int type, MegaChatRequestListener *listener)
{
    ...
    request->setPerformRequest([this, request]() { return performRequest_pushReceived(request); });
    requestQueue.push(request);
    waiter->notify();
}
...
MegaStringList* MegaChatApiImpl::getChatScreenDevices()
{
    ...
    {
        API_LOG_ERROR("%sFailed to get screen devices", getLoggingName());
    }
    return sl;
}
...
MegaStringList *MegaChatApiImpl::getChatVideoInDevices()
{
    ...
    {
        API_LOG_ERROR("%sFailed to get video-in devices", getLoggingName());
    }
    sdkMutex.unlock();
...
char *MegaChatApiImpl::getCameraDeviceIdSelected()
{
    ...
    {
        API_LOG_ERROR("%sFailed to get selected video-in device", getLoggingName());
    }
...
char* MegaChatApiImpl::getVideoDeviceNameById(const std::string& id) const
{
    ...
    if (!mClient || !mClient->rtc)
    {
        API_LOG_ERROR("%sgetVideoDeviceNameById: webrtc not initialized", getLoggingName());
        return nullptr;
    }
...
int MegaChatApiImpl::performRequest_rejectCall(MegaChatRequestPrivate* request)
{
    if (!mClient->rtc)
    {
        API_LOG_ERROR("%sREJECT_CALL - WebRTC is not initialized", getLoggingName());
        return MegaChatError::ERROR_ACCESS;
    }
    ...
    if (callId == MEGACHAT_INVALID_HANDLE)
    {
        API_LOG_ERROR("%sREJECT_CALL - invalid callId", getLoggingName());
        return MegaChatError::ERROR_ARGS;
    }
    ...
    if (!call)
    {
        API_LOG_ERROR("%sREJECT_CALL - There is not any call with that callId", getLoggingName());
        return MegaChatError::ERROR_NOENT;
    }
    ...
    if (!chatroom)
    {
        API_LOG_ERROR("%sREJECT_CALL - Chatroom has not been found", getLoggingName());
        return MegaChatError::ERROR_NOENT;
    }
...
int MegaChatApiImpl::performRequest_sendRingIndividualInACall(MegaChatRequestPrivate* request)
{
    ...
    if (chatId == MEGACHAT_INVALID_HANDLE)
    {
        API_LOG_ERROR("%sRing individual in call: invalid chat id", getLoggingName());
        return MegaChatError::ERROR_ARGS;
    }
    const auto userIdToCall = request->getUserHandle();
    if (userIdToCall == MEGACHAT_INVALID_HANDLE)
    {
        API_LOG_ERROR("%sRing individual in call: invalid user id", getLoggingName());
        return MegaChatError::ERROR_ARGS;
    }

    if (request->getParamType() <= 0)
    {
        API_LOG_ERROR("%sRing individual in call: invalid ring timeout", getLoggingName());
        return MegaChatError::ERROR_ARGS;
    }
    ...
    if (!chatroom)
    {
        API_LOG_ERROR("%sError: chat room with id %s not found", getLoggingName(), ID_CSTR(chatId));
        return MegaChatError::ERROR_NOENT;
    }
    ...
    if (!call)
    {
        API_LOG_ERROR("%sRing individual in call: no call found for chat id %s",
                      getLoggingName(),
                      ID_CSTR(chatId));
        return MegaChatError::ERROR_NOENT;
    }
    ...
    mClient->ringIndividualInACall(chatId, userIdToCall)
        ...
        .fail(
            [request, this](const ::promise::Error& err)
            {
                API_LOG_ERROR("%sError sending 'mcru' command to API: %s",
                              getLoggingName(),
                              err.what());
                MegaChatErrorPrivate* megaChatError =
                    new MegaChatErrorPrivate(err.msg(), err.code(), err.type());
...
void MegaChatApiImpl::setLimitsInCall(const MegaChatHandle chatid,
                                      ...
                                      MegaChatRequestListener* listener)
{
    ...
    limits->addMegaHandle(static_cast<MegaHandle>(divider));
...
bool MegaChatApiImpl::setIgnoredCall(MegaChatHandle chatId)
{
    if (!mClient->rtc)
    {
        API_LOG_ERROR("%sIgnore call - WebRTC is not initialized", getLoggingName());
        return false;
    }

    if (chatId == MEGACHAT_INVALID_HANDLE)
    {
        API_LOG_ERROR("%sIgnore call - Invalid chatId", getLoggingName());
        return false;
    }
    ...
    if (!call)
    {
        API_LOG_ERROR("%sIgnore call - Failed to get the call associated to chat room",
                      getLoggingName());
        return false;
    }
    ...
    if (call->isIgnored())
    {
        API_LOG_ERROR("%sIgnore call - Call is already marked as ignored", getLoggingName());
        return false;
    }
...
MegaChatCall *MegaChatApiImpl::getChatCall(MegaChatHandle chatId)
{
    ...
    if (!mClient->rtc)
    {
        API_LOG_ERROR("%sMegaChatApiImpl::getChatCall - WebRTC is not initialized",
                      getLoggingName());
        return chatCall;
    }
    ...
    if (chatId != MEGACHAT_INVALID_HANDLE)
    {
        ...
        {
            API_LOG_ERROR(
                "%sMegaChatApiImpl::getChatCall - Failed to get the call associated to chat room",
                getLoggingName());
            return chatCall;
        }
...
bool MegaChatApiImpl::isAudioLevelMonitorEnabled(MegaChatHandle chatid)
{
    if (chatid == MEGACHAT_INVALID_HANDLE)
    {
        API_LOG_ERROR("%sisAudioLevelMonitorEnabled - Invalid chatId", getLoggingName());
        return false;
    }
    ...
    {
        API_LOG_ERROR(
            "%sisAudioLevelMonitorEnabled - Failed to get the call associated to chat room",
            getLoggingName());
        return false;
    }
...
void MegaChatApiImpl::enableSpeakRequestSupportForCalls(const bool enable)
{
    ...
    {
        API_LOG_ERROR(
            "%sMegaChatApiImpl::enableSpeakRequestSupportForCalls - WebRTC is not initialized",
            getLoggingName());
        return;
    }
...
MegaChatApiImpl::getCall(const MegaChatHandle chatid, const std::string& msg, const bool ownPrivMod, MTristate waitingRoom)
{
    if (chatid == MEGACHAT_INVALID_HANDLE)
    {
        API_LOG_ERROR("%s%s - Invalid chatid", getLoggingName(), msg.c_str());
    ...
    if (!chatroom)
    {
        API_LOG_ERROR("%s%s - There is not any chatroom with chatid: %s",
                      getLoggingName(),
                      msg.c_str(),
                      karere::Id(chatid).toString().c_str());
    ...
    {
        API_LOG_ERROR("%s%s - Invalid chatroom with chatid: %s. Expected waiting room state: %s ",
                      getLoggingName(),
                      msg.c_str(),
                      karere::Id(chatid).toString().c_str(),
                      waitingRoom.get() ? "Enabled" : "Disabled");
        return std::make_pair(MegaChatError::ERROR_NOENT, nullptr);
    }
    ...
    if (!call)
    {
        API_LOG_ERROR("%s%s - There is not any call in that chatroom",
                      getLoggingName(),
                      msg.c_str());
        return std::make_pair(MegaChatError::ERROR_NOENT, nullptr);
    }
    ...
    if (call->getState() != rtcModule::kStateInProgress)
    {
        API_LOG_ERROR("%s%s - Call isn't in progress state", getLoggingName(), msg.c_str());
        return std::make_pair(MegaChatError::ERROR_ACCESS, nullptr);
    }

    if (ownPrivMod && !call->isOwnPrivModerator())
    {
        API_LOG_ERROR("%s%s - moderator role %s required for own user to perform this operation",
                      getLoggingName(),
                      msg.c_str());
        return std::make_pair(MegaChatError::ERROR_ACCESS, nullptr);
    }
...
void MegaChatApiImpl::addChatVideoListener(MegaChatHandle chatid,
                                           MegaChatHandle clientId,
                                           rtcModule::VideoResolution videoResolution,
                                           const int capturerType,
                                           MegaChatVideoListener* listener)
{
    ...
    if (clientId == MegaChatApi::LOCAL_CLIENT_ID_FOR_VIDEO &&
        (capturerType != MegaChatApi::TYPE_CAPTURER_SCREEN &&
         capturerType != MegaChatApi::TYPE_CAPTURER_VIDEO))
    {
        return;
    }

    assert(videoResolution != rtcModule::VideoResolution::kUndefined);
    SdkMutexGuard g(videoMutex);
    if (clientId == MegaChatApi::LOCAL_CLIENT_ID_FOR_VIDEO)
    {
        if (capturerType == MegaChatApi::TYPE_CAPTURER_VIDEO)
        {
            mLocalCameraVideoListeners[chatid].insert(listener);
        }
        else if (capturerType == MegaChatApi::TYPE_CAPTURER_SCREEN)
        {
            mLocalScreenVideoListeners[chatid].insert(listener);
        }
        else
        {
            API_LOG_ERROR("%saddChatVideoListener: Invalid capturer type: %i",
                          getLoggingName(),
                          capturerType);
            assert(false);
            return;
        ...
        marshallCall(
            [this, chatid, capturerType]()
            {
                ...
                if (mClient && mClient->rtc)
                {
                    if (capturerType == MegaChatApi::TYPE_CAPTURER_VIDEO)
                    {
                        if (!mClient->rtc->hasLocalCameraRenderer(chatid))
                        {
                            mClient->rtc->addLocalCameraRenderer(
                                chatid,
                                new MegaChatVideoReceiver(this,
                                                          chatid,
                                                          rtcModule::VideoResolution::kHiRes));
                        }
                    }
                    else if (capturerType == MegaChatApi::TYPE_CAPTURER_SCREEN)
                    {
                        if (!mClient->rtc->hasLocalScreenRenderer(chatid))
                        {
                            mClient->rtc->addLocalScreenRenderer(
                                chatid,
                                new MegaChatVideoReceiver(this,
                                                          chatid,
                                                          rtcModule::VideoResolution::kHiRes));
...
void MegaChatApiImpl::removeChatVideoListener(MegaChatHandle chatid,
                                              MegaChatHandle clientId,
                                              rtcModule::VideoResolution videoResolution,
                                              const int capturerType,
                                              MegaChatVideoListener* listener)
{
    ...
    if (clientId == MegaChatApi::LOCAL_CLIENT_ID_FOR_VIDEO &&
        (capturerType != MegaChatApi::TYPE_CAPTURER_SCREEN &&
         capturerType != MegaChatApi::TYPE_CAPTURER_VIDEO))
    {
        API_LOG_ERROR("%sremoveChatVideoListener: Invalid capturer type: %i",
                      getLoggingName(),
                      capturerType);
        return;
    }
    ...
    if (clientId == MegaChatApi::LOCAL_CLIENT_ID_FOR_VIDEO)
    {
        auto& listeners = capturerType == MegaChatApi::TYPE_CAPTURER_VIDEO
                                ? mLocalCameraVideoListeners
                                : mLocalScreenVideoListeners;

        if (auto it = listeners.find(chatid);
            it != listeners.end())
        {
            ...
            if (videoListenersSet.empty())
            {
                ...
                marshallCall(
                    [this, chatid, capturerType]()
                    {
                        ...
                        {
                            capturerType == MegaChatApi::TYPE_CAPTURER_VIDEO
                                ? mClient->rtc->removeLocalCameraRenderer(chatid)
                                : mClient->rtc->removeLocalScreenRenderer(chatid);
                        }
                    },
                    this);
            }
    ...
    else if (videoResolution == rtcModule::VideoResolution::kHiRes || videoResolution == rtcModule::VideoResolution::kLowRes)
    {
        ...
        auto itRes = listeners.find(chatid);
        if (itRes != listeners.end())
        {
            MegaChatPeerVideoListener_map& videoListenersMap = itRes->second;
            ...
            if (auxit != videoListenersMap.end())
            {
                ...
                {
                    // if MegaChatVideoListener_set is empty, remove entry from
                    // MegaChatPeerVideoListener_map
            ...
            {
                // if MegaChatPeerVideoListener_map is empty, remove entry from listeners
                // map
                listeners.erase(chatid);
            }
        }
    ...
    {
        API_LOG_ERROR("%sremoveChatVideoListener: Invalid video resolution type: %i",
                      getLoggingName(),
                      videoResolution);
        assert(false);
    }
...
int MegaChatApiImpl::getMessageReactionCount(MegaChatHandle chatid, MegaChatHandle msgid, const char *reaction)
{
    ...
    if (!msg)
    {
        API_LOG_ERROR("%sChatroom or message not found", getLoggingName());
        return -1;
    }
...
int MegaChatApiImpl::getCurrentInputVideoTracksLimit() const
{
    if (!mClient)
    {
        API_LOG_ERROR("%sgetCurrentInputVideoTracksLimit: Karere client not initialized",
                      getLoggingName());
    ...
    {
        API_LOG_ERROR(
            "%sgetCurrentInputVideoTracksLimit: Invalid value for simultaneous input video tracks",
            getLoggingName());
        return MegaChatApi::INVALID_CALL_VIDEO_SENDERS;
    }
...
bool MegaChatApiImpl::setCurrentInputVideoTracksLimit(const int numInputVideoTracks)
{
    if (!mClient)
    {
        API_LOG_ERROR("%ssetCurrentInputVideoTracksLimit: Karere client not initialized",
                      getLoggingName());
        return false;
    }
    ...
    {
        API_LOG_DEBUG("%ssetCurrentInputVideoTracksLimit: Invalid value for simultaneous input "
                      "video tracks: %d",
                      getLoggingName(),
                      auxNumInputVideoTracks);
        return false;
    }
...
{
    API_LOG_DEBUG("%sKarere initialization state has changed: %d", getLoggingName(), newState);

    if (newState == karere::Client::kInitErrSidInvalid)
    {
        API_LOG_WARNING("%sInvalid session detected (API_ESID). Logging out...", getLoggingName());
        logout();
        return;
    }
...
void MegaChatApiImpl::onPresenceChanged(Id userid, Presence pres, bool inProgress)
{
    if (inProgress)
    {
        API_LOG_INFO("%sMy own presence is being changed to %s", getLoggingName(), pres.toString());
    }
    else
    {
        API_LOG_INFO("%sPresence of user %s has been changed to %s",
                     getLoggingName(),
                     ID_CSTR(userid),
                     pres.toString());
    }
...
void MegaChatRequestPrivate::setMegaChatPeerList(MegaChatPeerList *peerList)
{
    ...
    mPeerList =
        peerList ? peerList->copy() : static_cast<MegaChatPeerList*>(new MegaChatPeerListPrivate());
}
...
MegaChatCallPrivate::MegaChatCallPrivate(const rtcModule::ICall &call)
{
    ...
    mCallWillEndTs = call.getCallWillEndTs();
    mCallLimits = call.getCallLimits();
...
MegaChatTimeStamp MegaChatCallPrivate::getCallWillEndTs() const
{
    return static_cast<MegaChatTimeStamp>(mCallWillEndTs);
}
...
int MegaChatCallPrivate::getCallUsersLimit() const
{
    return mCallLimits.numUsers;
}

int MegaChatCallPrivate::getCallClientsLimit() const
{
    return mCallLimits.numClients;
}

int MegaChatCallPrivate::getCallClientsPerUserLimit() const
{
    return mCallLimits.numClientsPerUser;
}

sfu::SfuInterface::CallLimits MegaChatCallPrivate::getCallLimits() const
{
    return mCallLimits;
}
...
void* MegaChatVideoReceiver::getImageBuffer(unsigned short width, unsigned short height, int sourceType, void*& userData)
{
...
void MegaChatVideoReceiver::frameComplete(void *userData)
{
    ...
    mChatApi->fireOnChatVideoData(mChatid, mClientId, frame->width, frame->height, frame->sourceType, (char *)frame->buffer, mClientId ? mVideoResolution : rtcModule::VideoResolution::kHiRes);
...
MegaChatErrorPrivate::MegaChatErrorPrivate(const ::promise::Error& err):
    ::promise::Error(err.msg(), err.code(), err.type())
{
    setHandled();
}
