...
CREATE TABLE chats(chatid int64 unique primary key, shard tinyint,
    ...
    mode tinyint default 0, unified_key blob, rsn blob);
...
CREATE TABLE chat_reactions(chatid int64 not null, msgid int64 not null, userid int64 not null,
    reaction text, UNIQUE(chatid, msgid, userid, reaction));
