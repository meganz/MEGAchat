CREATE TABLE sending(rowid integer primary key autoincrement, msgid int64, keyid int, chatid int64 not null, type tinyint, ts int, updated smallint, data blob, opcode smalling not null, out_cmd blob);
...
CREATE TABLE contacts(userid int64 PRIMARY KEY, email text, since int64 not null default 0);
...
CREATE TABLE history(idx int not null, chatid int64 not null, msgid int64 primary key, userid int64, keyid int not null, type tinyint, updated smallint, ts int, data blob);
