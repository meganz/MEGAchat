...
protected:
	...
	*/
    ...
    std::shared_ptr<PresenceMessage> composePresenceMessage(strophe::Stanza stanza) {
        ...
        if(from.find("@conference") != std::string::npos) {
            ...
            presenceMessage = std::shared_ptr<PresenceMessage>(
                                   new PresenceMessage(
                                           stanza.attr("to"),
                                           fromJid,
                                           roomJid,
										   (type != NULL) ? stanza.attr("type") : std::string("available")
										   ));
        }
	...
	*/
    std::shared_ptr<IncomingMessage> composeIncomingMessesage(strophe::Stanza stanza)
    {
        ...
        inviteMessage = std::shared_ptr<IncomingMessage>(
                new IncomingMessage(
                    std::string(stanza.attr("to")),
                    fromJid,
                    roomJid,
                    (type.compare("groupchat") == 0) ? IncomingMessage::GROUPCHAT : IncomingMessage::CHAT,
                    std::string(""),
                    std::string(intext)));
	...
	*/
    ...
    {
        std::string Jid(xmpp_conn_get_bound_jid(*conn));
        return Jid.substr(0, Jid.find("@"));
    }
...
public:
    ...
    void handleDataMessage(std::shared_ptr<IncomingMessage> &message)
    {
        ...
        else
        {
            ...
            message_bus::SharedMessageBus<M_BUS_PARAMS>::
                getMessageBus()->alertListeners(messName, busMessage);
        }
    }
    ...
    void addOtherUser(const std::string &roomId, const std::string &otherJid)
    {
        ...
        std::string m("User");
        m.append(member->getId());
        m.append(" has entered the room");
    ...
    */
     ...
     */
    promise::Promise<int> pingPeer(const std::string& peerJid, int intervalSec = 100)
    {
    ...
    void joinRoom(const std::string& roomJid, const std::string& password = "", const MessageMeta& meta = MessageMeta())
    {
        ...
        if( chatRooms.find(roomJid) == chatRooms.end())
        {
            ...
            std::shared_ptr<ChatRoom<M, GM, SP, EH>> chatRoom =
                    std::shared_ptr<ChatRoom<M, GM, SP, EH>>(new ChatRoom<M, GM, SP, EH>(*this, roomJid, meta.participants));
...
protected:
    /*mEmail {string}, client's email address*/
    std::string mEmail;
    /*mPassword {string}, client's password*/
    ...
    /*contactList {ContactList}, client's contact list*/
    ContactList contactList;
    /*handler for webrtc event*/
