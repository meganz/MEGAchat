...
public:
    ...
    virtual chatd::Idx getIdxOfMsgid(const chatd::Id& msgid)
    {
    ...
    {
        SqliteStmt stmt(mDb, "select count(*) from history where (userid != ?) and (idx > ?) and edits = 0");
        stmt << mMessages->client().userId() << idx;
        stmt.step();
